...
📽️ **Video Link:** `[INSERT GOOGLE DRIVE LINK HERE]`
...
Finally, every completed order generates a **cryptographic verifiable receipt**, an HMAC-SHA256 signature computed over the canonical order fields. If anyone tampers with the stored price or quantity even by ₹1, the hash recomputed at verification time will instantly mismatch, triggering a security alert.
...
- **Multi-Merchant Interoperability** — Two standardized catalogs: Apex Outfitters & Urban Trail Co.
- **Full Audit Trail** — Every action logged to MongoDB `audit_logs` collection in real time

---

## 🗂️ Demo Data Location

All seed/demo data is hardcoded directly in the backend and loaded into MongoDB Atlas (or the in-memory fallback) on first startup.
...
| **Excel data reference sheet** (for judges) | `Razorpay_Agentic_Commerce_Data_Sheet.xlsx` (root folder) |
...
├── Razorpay_Agentic_Commerce_Data_Sheet.xlsx   # Judge reference
├── docker-compose.yml
└── README.md
```
...
# RAZORPAY_KEY_SECRET=...
# RECEIPT_SIGNING_KEY=your_secret_key
```
...
✅ Order Value Cap (>₹10k) Check: REJECTED
...
Open `http://localhost:5173` and try these in the **Conversational Checkout** tab:
...
| `"Order the Apex Expedition 8000 Parka"` | Rejected — ₹14,999 exceeds ₹10,000 automated order limit |
...
Create a `.env` file inside the `backend/` folder using this reference:
...
|---|---|---|
| `GET` | `/api/health` | Health check + merchant list |
